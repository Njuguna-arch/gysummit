...
function checkEnv() {
  ...
  if (!process.env.AT_API_KEY || !process.env.AT_USERNAME) {
    warnings.push("SMS notifications are not configured (AT_API_KEY/AT_USERNAME) — turning on SMS in Announcements settings will just log instead of sending.");
  }
  if (!process.env.WHATSAPP_PHONE_NUMBER_ID || !process.env.WHATSAPP_ACCESS_TOKEN) {
    warnings.push("WhatsApp notifications are not configured (WHATSAPP_PHONE_NUMBER_ID/WHATSAPP_ACCESS_TOKEN) — turning on WhatsApp in Announcements settings will just log instead of sending.");
  }
